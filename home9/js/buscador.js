...
async function searchTmdb(query) {
  ...
  // Prioritize language from index.html
  const htmlLang = document.documentElement.lang;
  if (htmlLang && htmlLang.startsWith('es')) {
    // If html lang is Spanish, try to use navigator.language if it's also Spanish, otherwise default to es-ES
    if (navigator.language && navigator.language.startsWith('es')) {
      language = navigator.language;
    } else {
      language = 'es-ES'; // Fallback to a common Spanish variant
    }
  } else if (navigator.language && navigator.language.startsWith('es')) {
    // If html lang is not Spanish, but navigator.language is, use it
    language = navigator.language;
  }
  // If neither is Spanish, it defaults to 'es-ES'
...
export async function initBuscador(appMainContent) {
      ...
      if (isSearching) {
        return;
          ...
          if (query.length > 2) {
            ...
            try {
              // Use a default language for general search, or try to infer from browser
  ...
  const displayResults = (results) => {
    ...
    results.forEach((item) => {
        // Ensure customTitle and customLogoUrl are present for buildCard
        if (!item.customTitle) item.customTitle = '';
        if (!item.customLogoUrl) item.customLogoUrl = '';
  ...
  let detailedMedia = [];
    ...
    const filteredMedia = detailedMedia.filter((item) => {
      // Prioritize customTitle if available, otherwise use TMDB title
      const title = (item.customTitle || item.title || item.name || item.original_title || item.original_name || '').toString();
  ...
  async function fetchDetailsForLocalItems(items) {
    const detailedItems = await Promise.all(
      items.map(async (item) => {
        const language = item.language || "es-ES"; // Get language from item or default
        const url = `https://api.themoviedb.org/3/${item.tipo}/${item.id}?api_key=${tmdbApiKey}&language=${language}`;
        try {
          const response = await fetch(url);
