...
// Función para exportar favoritos usando un CDN para el archivo
export const exportFavorites = async () => {
  ...
  try {
    ...
    // Usar filebin.net como servicio de almacenamiento temporal
    const formData = new FormData();
    formData.append('file', blob, 'yami-favoritos.json');

    // Usar un servicio de almacenamiento temporal (filebin.net)
    const response = await fetch('https://filebin.net/yamiapp/favorites', {
      method: 'POST',
      body: formData
    });
    ...
    downloadLink.download = 'yami-favoritos.json';
    ...
    alert('Error al exportar favoritos. Por favor intenta de nuevo.');
  }
};
...
async function createFavoriteCard(item) {
  ...
  // Add export button
  if (!document.getElementById('export-favorites-btn')) {
    const exportBtn = document.createElement('button');
    exportBtn.id = 'export-favorites-btn';
    exportBtn.className = 'bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg shadow-md transition-colors duration-200 flex items-center space-x-2 mx-4';
    exportBtn.innerHTML = `
    ...
    document.querySelector('.flex.items-center.justify-center.space-x-4').appendChild(exportBtn);
  }
