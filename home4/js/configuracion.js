...
// --- Favorites Export/Import Logic (Encryption/Decryption removed as per user request) ---
...
const setupFavoritesManagement = () => {
  ...
  exportButton.addEventListener('click', async () => {
    try {
      ...
      if (!favorites || favorites === '[]') {
        showToast("No hay favoritos para exportar.", "https://cdn.jsdelivr.net/gh//geminiproai37-blip/dsfdfd@latest/HOME/AI7yqKC5Ov0B2.webp"); // Use a neutral GIF
        return;
      }

      const dataToExport = await encryptData(favorites); // encryptData now just returns the data
      const blob = new Blob([dataToExport], { type: 'text/plain' });
      const url = URL.createObjectURL(blob);

      const a = document.createElement('a');
      a.href = url;
      a.download = 'yamiLat_favorites.yamilatfav';
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);

      showToast("Favoritos exportados con éxito.", "https://cdn.jsdelivr.net/gh//geminiproai37-blip/dsfdfd@latest/HOME/MdLFOyVZtoUPm.webp"); // New GIF for export success
    } catch (error) {
      console.error('Error exporting favorites:', error);
      showToast("Error al exportar favoritos.", "https://cdn.jsdelivr.net/gh//geminiproai37-blip/dsfdfd@latest/HOME/MdLFOyVZtoUPm.webp"); // Error GIF
    }
  });
  ...
  fileInput.addEventListener('change', async (event) => {
    ...
    if (!file.name.endsWith('.yamilatfav')) {
      showToast("Archivo inválido. Por favor, selecciona un archivo .yamilatfav", "https://cdn.jsdelivr.net/gh//geminiproai37-blip/dsfdfd@latest/HOME/MdLFOyVZtoUPm.webp");
      return;
    }
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (!importedData) {
          throw new Error("El archivo está vacío o es ilegible.");
        }
        const parsedFavorites = JSON.parse(importedData); // Directly parse the imported data

        // Basic validation to ensure it's an array
        if (!Array.isArray(parsedFavorites)) {
          throw new Error("Invalid favorites data format after import.");
        }

        localStorage.setItem('yamiLatFavorites', JSON.stringify(parsedFavorites));
        showToast("Favoritos importados con éxito.", "https://cdn.jsdelivr.net/gh//geminiproai37-blip/dsfdfd@latest/HOME/ezgif-895ea998200bb6.webp"); // New GIF for import success
        ...
        // Also refresh the main content display to ensure any other parts of the app that depend on favorites are updated
        ...
        let userMessage = `Error al importar favoritos: ${error.message}`;
        if (error.message.includes("Invalid favorites data format")) {
            userMessage = "Error al importar: El archivo tiene un formato inválido o está dañado.";
        }
        showToast(userMessage, "https://cdn.jsdelivr.net/gh//geminiproai37-blip/dsfdfd@latest/HOME/ezgif-895ea998200bb6.webp"); // Use an error GIF
      } finally {
  ...
  sendBugReportButton.addEventListener("click", async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showToast("Reporte enviado. Lo solucionaremos pronto.", "https://i.imgur.com/2g00000.gif"); // Placeholder for a different success GIF
...
document.addEventListener('DOMContentLoaded', () => {
  if (!document.getElementById('toast-notification-container')) {
    document.body.insertAdjacentHTML('beforeend', `
      <div id="toast-notification-container" class="fixed bottom-16 left-1/2 transform -translate-x-1/2 z-[1000] flex flex-col space-y-2"></div>
    `);
  }
});
