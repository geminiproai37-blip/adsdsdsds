// js/adultContent.js
import { createHeroSlider, createContentSlider, createLatestEpisodesSlider, fetchMediaDetails, buildRatingBadge } from "./script.js"; // Import new slider and fetch function, and getProgressData
...
async function createSecicondEdestacadoSection(explosivoData) {
  ...
  const language = explosivoData.language || "es-ES"; // Get language from explosivoData or default
  const mediaDetails = await fetchMediaDetails(apiBaseUrl, explosivoData.tipo, explosivoData.id, null, null, language);
...
export async function loadAdultContentSection(allMediaDetails = [], imageBaseUrl) {
  ...
  if (adultContentTemplate) {
    ...
    const mainElement = appMainContent.querySelector("#main3-content");
    ...
    adultSlidersSection.classList.add('extra-badge-spacing');
    ...
    if (featuredAdultContent.length > 0) {
      ...
      featuredSection.className = "py-8 extra-badge-spacing"; // Add some padding
        ...
        featuredContent.forEach(item => {
          ...
          if (item && (item.poster_path || item.backdrop_path)) {
            ...
            if (item.languages && item.languages.length > 0) {
              ...
              languageTagsContainer.className = "language-tags-container";
              languageTagsContainer.innerHTML = `
                ${item.languages.map(lang => `
                  <div class="language-tag-item">
                    <span class="language-tag language-${lang.toLowerCase()}">${lang}</span>
                    ${getFlagEmoji(lang)}
                  </div>
                `).join('')}
              `;
              contentCard.appendChild(languageTagsContainer);
            }
