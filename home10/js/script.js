...
const telegramBotToken = "75015928444:AAFR8K1wZEdie8g8F4FY3rVtKyM3EEZ8xg0"; // Using the one from buscador/config.js
...
import { initConfiguracionPageContent } from "./configuracion.js"; // Import initConfiguracionPageContent
...
export function createHeroSlider(backdropBaseUrl, items) {
  ...
  // Container for badges and flags in hero slider
  const overlayContainer = document.createElement("div");
  overlayContainer.className = "absolute top-2 right-2 flex flex-col items-end space-y-1 z-20"; // Added z-index
  ...
  items.forEach((item, index) => {
    if (item && item.poster_path) {
      ...
      if (item.customLogoUrl) {
        console.log(`createHeroSlider for item ID ${item.id}: Using customLogoUrl: ${item.customLogoUrl}`);
        const logo = document.createElement("img");
        logo.src = item.customLogoUrl;
        logo.alt = `${item.customTitle || item.title || item.name} logo`;
        logo.className = "hero-title-logo";
        slide.appendChild(logo);
      } else if (item.logo_path) {
        console.log(`createHeroSlider for item ID ${item.id}: Using logoPath: ${item.logo_path}`);
        const logo = document.createElement("img");
      ...
      // Container for badges and flags in hero slider
      slide.appendChild(overlayContainer);


      if (item.vote_average) {
        const ratingBadgeHTML = buildRatingBadge(item.vote_average);
        if (ratingBadgeHTML) {
          overlayContainer.innerHTML += ratingBadgeHTML;
        }
      }
...
export function createContentSlider(imageBaseUrl, title, items, isAdultSection = false) {
  ...
  items.forEach((item) => {
    if (item && (item.poster_path || item.backdrop_path)) {
      ...
      // Use TMDB poster path for the main poster image. Custom logo is handled separately if needed.
      posterImage.src = imageBaseUrl + (item.still_path || item.poster_path);
      posterImage.alt = item.customTitle || item.name || item.title;
      ...
      deleteButton.title = `Eliminar ${item.customTitle || item.title || item.name}`;
      ...
      contentTitle.textContent = item.customTitle || item.title || item.name;
      ...
      if (item.vote_average) {
        contentCard.insertAdjacentHTML('beforeend', buildRatingBadge(item.vote_average));
      }
...
export function createLatestEpisodesSlider(imageBaseUrl, title, items, titleColorClass = 'text-orange-500', isAdultSection = false) {
  ...
  items.forEach((item) => {
    if (item && (item.still_path || item.backdrop_path || item.poster_path)) {
      ...
      contentCard.addEventListener("click", () => {
        // If a custom URL is defined, open it exclusively.
        if (item.customUrl && item.customUrl !== '#') {
          window.open(item.customUrl, '_self');
        }
        // Do nothing if customUrl is not defined or is '#'
      });
      ...
      episodeImage.src = item.customLogoUrl || imageBaseUrl + (item.still_path || item.backdrop_path || item.poster_path);
      episodeImage.alt = item.customTitle || item.series_name || item.name || item.title;
      episodeImage.className = "w-full h-full object-cover";
      episodeImage.loading = "lazy";
      imageContainer.appendChild(episodeImage);

      // Container for badges and flags in latest episodes slider
      const overlayContainer = document.createElement("div");
      overlayContainer.className = "absolute top-2 right-2 flex flex-col items-end space-y-1 z-20"; // Added z-index
      ...
      if (item.vote_average) {
        ...
        ratingContainer.innerHTML = buildRatingBadge(item.vote_average);
        if (ratingContainer.firstChild) {
            overlayContainer.appendChild(ratingContainer.firstChild);
        }
      }
      ...
      bottomInfoBar.innerHTML = `
        <p class="text-white text-xs font-semibold truncate w-full">${item.customTitle || item.series_name || item.name || 'Series Name N/A'}</p>
...
export function createContinueWatchingSlider(imageBaseUrl, title, items, isAdultSection = false) {
  ...
  items.forEach((item) => {
    if (item && (item.poster_path || item.backdrop_path)) {
      ...
      contentCard.addEventListener("click", () => {
        let targetUrl;
        // Define the default series URL pattern dynamically
        const defaultSeriesUrlPattern = `#series/${item.id}/season/${item.season}/episode/${item.episode}`;

        // Prioritize customUrl if it's provided, not a placeholder, and not the default pattern itself
        if (item.customUrl && item.customUrl !== '#' && item.customUrl !== 'tem.url' && item.customUrl !== defaultSeriesUrlPattern) {
          targetUrl = item.customUrl;
        } else if (item.url) { // Fallback to item.url from contentDefinitionsDb
          targetUrl = item.url;
        } else { // Final fallback to the default pattern
          targetUrl = defaultSeriesUrlPattern;
        }
...
export function createRankedSlider(imageBaseUrl, title, items) {
  ...
  items.forEach((item, index) => {
    if (item && item.poster_path) {
      ...
      const posterImage = document.createElement("img");
      posterImage.src = item.customLogoUrl || imageBaseUrl + item.poster_path;
      posterImage.alt = item.customTitle || item.name || item.title;
...
function createMovieGrid(imageBaseUrl, movies) {
  ...
  movieGrid.className = "grid grid-cols-3 gap-4 relative";
  ...
  movies.forEach((movie) => {
    if (movie && movie.poster_path) {
      ...
      if (movie.vote_average) {
        overlayContainer.insertAdjacentHTML('beforeend', buildRatingBadge(movie.vote_average));
      }

      const movieTitle = document.createElement("p");
      movieTitle.className = "text-sm mt-2 truncate w-full";
      movieTitle.textContent = movie.customTitle || movie.title; // Prioritize customTitle
...
window.createSeriesGrid = function (imageBaseUrl, series, languages) {
  const seriesGrid = document.createElement("div");
  seriesGrid.id = "series-grid";
  seriesGrid.className = "grid grid-cols-3 gap-4 relative";
  ...
  series.forEach((item) => {
    if (item && item.poster_path) {
      ...
      const overlayContainer = document.createElement("div");
      overlayContainer.className = "absolute inset-0 pointer-events-none"; // Simplified to just cover the card
      seriesCard.appendChild(overlayContainer);

      if (item.rank) {
        const rankOverlay = document.createElement("div");
        rankOverlay.className = "rank-overlay absolute top-1 right-1"; // Positioned top-right, reduced space
        rankOverlay.textContent = item.rank;
        overlayContainer.appendChild(rankOverlay);
      }

      if (item.isAdult) {
        const adultBadge = document.createElement("div");
        adultBadge.className = "adult-badge"; // CSS will handle positioning (top-right)
        adultBadge.textContent = "+18";
        overlayContainer.appendChild(adultBadge);
      }
      ...
      if (item.vote_average) {
        ...
        ratingBadgeContainer.className = "rating-badge-container"; // CSS will handle positioning (top-left)
        ratingBadgeContainer.innerHTML = buildRatingBadge(item.vote_average);
        overlayContainer.appendChild(ratingBadgeContainer);
      }

      const seriesTitle = document.createElement("p");
      seriesTitle.className = "text-sm mt-2 truncate w-full";
      seriesTitle.textContent = item.customTitle || item.title || item.name; // Prioritize customTitle
...
export function buildRatingBadge(rating) {
  if (!rating || rating === 0) {
    return '';
  }
  return `
...
export function buildCard(imageBaseUrl, item) {
  ...
  const title =
    item.customTitle || item.title || item.name || item.original_title || item.original_name;
  const releaseDate = item.release_date || item.first_air_date;
  const year = releaseDate ? new Date(releaseDate).getFullYear() : "N/A";

  const ratingBadge = buildRatingBadge(item.vote_average);
  ...
  card.innerHTML = `
          ...
          ${
            ...
            ${ratingBadge}
            ${
              item.customLogoUrl // Prioritize custom logo URL
                ? `<img src="${item.customLogoUrl}" alt="${item.customTitle || title} logo" class="absolute inset-0 m-auto h-12 object-contain" />`
              : item.logo_path // Then TMDB logo path
                ? `<img src="${imageBaseUrl}${item.logo_path}" alt="${title} logo" class="absolute inset-0 m-auto h-12 object-contain" />`
                : `<h3 class="absolute inset-0 m-auto text-white text-sm font-bold flex items-center justify-center text-center">${item.customTitle || title}</h3>` // Fallback to custom title or general title
            }
          </div>
          <div class="p-3 flex flex-col justify-center flex-grow">
            <h3 class="text-base font-semibold text-white">${item.customTitle || title}</h3>
...
export function buildSliderPoster(imageBaseUrl, item) {
  ...
  try {
    // 1. Fetch main media details
    let mainDetails = {};
    let episodeDetails = null;
    let episodeStills = [];
    const languagePriority = ["es-MX", "es-ES", "es", "en", "ja"];
    for (const lang of languagePriority) {
      const url = `${apiBaseUrl}/${type}/${id}?api_key=${tmdbApiKey}&language=${lang}`;
      const response = await fetch(url);
      if (response.ok) {
        mainDetails = await response.json();
        if (mainDetails.name || mainDetails.overview) {
          break;
        }
      }
    }
    ...
    if (!mainDetails.id) {
      // Fallback without language if no details found
    ...
    // Select logo from main images with strict language priority
    let logoPath = null;
    if (mainImagesData.logos?.length > 0) {
      const orderedLanguagePriority = ["es-MX", "es-ES", "es", "en", "ja"];
      let foundLogo = null;

      // Iterate through the strict priority list
      for (const langCode of orderedLanguagePriority) {
        foundLogo = mainImagesData.logos.find(logo => logo.iso_639_1 === langCode);
        if (foundLogo) {
          logoPath = foundLogo.file_path;
          break; // Found the highest priority logo, stop searching
        }
      }

      // If no logo found with strict priority, look for logos with null or 'xx' language code
      if (!logoPath) {
        foundLogo = mainImagesData.logos.find(logo => logo.iso_639_1 === null || logo.iso_639_1 === "xx");
        if (foundLogo) {
          logoPath = foundLogo.file_path;
        }
      }

      // If still no logo found, fallback to the very first logo available
      if (!logoPath && mainImagesData.logos.length > 0) {
        logoPath = mainImagesData.logos[0].file_path;
      }
    }
    console.log(`fetchMediaDetails for ID ${id}, Type ${type}: Selected logoPath: ${logoPath}`);
    ...
    const combinedDetails = {
      ...
      // The 'languages' property will be merged from window.contentDefinitionsFromDb in fetchAllMediaDetailsRaw
    };
...
async function initFavoritos() {
  ...
  const favoritosTemplateElement = document.getElementById("favoritos-template");
  if (favoritosTemplateElement) {
    favoritesPageContainer.innerHTML = favoritosTemplateElement.innerHTML;
  } else {
    console.error("Favoritos template not found.");
    return;
  }
  mainContentDiv.appendChild(favoritesPageContainer);

  // Initialize the favorites page from favoritos.js
...
window.configToggleStates = {
  isAdvancedSettingsOpen: false,
  isInfoReportsOpen: false,
};
...
async function handleNavigation() {
  ...
  // Capture configuration toggle states BEFORE clearing appMainContent
  if (hash === "configuracion") {
    const advancedSettingsContent = document.getElementById("advanced-settings-content");
    const infoReportsContent = document.getElementById("info-reports-content");
    window.configToggleStates.isAdvancedSettingsOpen = advancedSettingsContent ? !advancedSettingsContent.classList.contains("hidden") : false;
    window.configToggleStates.isInfoReportsOpen = infoReportsContent ? !infoReportsContent.classList.contains("hidden") : false;
    console.log("handleNavigation: Captured configToggleStates:", window.configToggleStates);
  } else {
    // Reset states if navigating away from configuracion
    window.configToggleStates.isAdvancedSettingsOpen = false;
    window.configToggleStates.isInfoReportsOpen = false;
  }
  ...
  if (hash.startsWith("series/")) {
    ...
    console.log("Adult Content Enabled (localStorage):", localStorage.getItem("adultContentEnabled"));
    console.log("window.allMediaDetails for adult content:", window.allMediaDetails);
    ...
    // Pass the current state of the toggles to initConfiguracionPageContent
    initConfiguracionPageContent(appMainContent, {
      isAdvancedSettingsOpen: configToggleStates.isAdvancedSettingsOpen,
      isInfoReportsOpen: configToggleStates.isInfoReportsOpen,
    });
  } else {
    // Default to home
...
window.refreshMainContentDisplay = async (isAdultContentEnabled) => {
  ...
  const currentScrollY = window.scrollY; // Capture current scroll position
  ...
  const fetchPromises = window.contentDefinitionsFromDb.map(async (item) => {
    ...
    // Merge details from TMDB first, then override with properties from contentDefinitionsFromDb (item)
    // This ensures 'languages' from item takes precedence.
    return details ? { ...details, ...item, type: item.tipo } : null;
  });
...
function applyAdultContentFilterAndSetGlobalMedia() {
  ...
  // Ensure customTitle and customLogoUrl are present for components like buildCard
  filteredMedia = filteredMedia.map(item => ({
    ...item,
