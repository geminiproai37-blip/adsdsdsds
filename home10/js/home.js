...
@media (min-width: 1024px) {
  .hero-title-logo {
    ...
    /* These styles are now primarily controlled by the main styles.css */
    .language-tag-item {
      display: flex;
      align-items: center;
      gap: 5px;
  ...
  explosivoSection.innerHTML = `
    ...
    <div class="relative z-20 flex flex-col items-center p-2 max-w-2xl mx-auto">
      ...
      <h2 class="text-2xl md:text-4xl lg:text-5xl text-white leading-tight mb-3 drop-shadow-md">
        ${explosivoData.customTitle || mediaDetails.title || mediaDetails.name}
      </h2>
...
export async function initHome(targetElement) {
  ...
  slidersSection.classList.add('extra-badge-spacing');
  ...
  console.log("All media details for home page:", window.allMediaDetails);
  console.log("Hero items to display (initial slice):", homeContent);

  // Hero Slider
  let heroItemsToDisplay = homeContent.slice(0, 7);

  // Fetch full details for hero slider items to get logos and backdrops
  heroItemsToDisplay = await Promise.all(
    heroItemsToDisplay.map(async (item) => {
      const language = "de"; // Force language to German for testing
      const details = await fetchMediaDetails(apiBaseUrl, item.tipo, item.id, null, null, language);
      
      // Ensure backdrop_path is present, prioritizing details, then item.
      // This helps if fetchMediaDetails fails or doesn't return backdrop_path,
      // but the original item might have it.
      const mergedItem = { ...item };
      if (details) {
        Object.assign(mergedItem, details);
      }
      // Explicitly ensure backdrop_path is set if available in original item and not in details
      if (!mergedItem.backdrop_path && item.backdrop_path) {
        mergedItem.backdrop_path = item.backdrop_path;
      }
  ...
  if (window.latestEpisodes) {
    ...
    const detailedLatestEpisodes = await Promise.all(
      nonAdultLatestEpisodes.map(async (item) => {
        const originalContentDef = window.contentDefinitionsFromDb.find(def => def.id === item.id && def.tipo === item.tipo);
        const language = originalContentDef ? originalContentDef.language || "es-ES" : "es-ES"; // Get language from original definition or default
        const details = await fetchMediaDetails(apiBaseUrl, item.tipo, item.id, item.season, item.episode, language);
        return details ? { 
          ...item, 
          ...details,
          language: language, // Explicitly add language from original definition
  ...
  uniqueCategories.forEach((category) => {
    ...
    if (itemsForCategory.length > 0) {
      ...
      const sliderElement = createContentSlider(imageBaseUrl, title, itemsForCategory, false, true); // showRating is true for all content sliders now
      if (category === 'series') {
        sliderElement.classList.add('series-popular-slider'); // Add a specific class for targeting
      }
      slidersSection.appendChild(sliderElement);
    }
  });
