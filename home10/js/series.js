    ...
    const loadPoster = (entries, observer) => {
        ...
        intersectingEntries.forEach((entry, index) => {
            ...
            setTimeout(() => {
                ...
                const posterFragment = window.createSeriesGrid(imageBaseUrl, [mediaData], mediaData.languages);
                ...
                if (posterElement) {
                    ...
                    // Ensure the overlayContainer and its contents (like rating-badge) are present
                    // The createSeriesGrid function in script.js already handles adding overlayContainer
                    // and rating-badge. So, we just need to ensure the whole posterElement is used.
...
filteredMedia.forEach((media) => {
  // Create a new object for display, ensuring the 'name' property is correctly set.
  // Prioritize 'personalizedTitle' if it exists, otherwise use the original 'name'.
  const mediaForDisplay = {
    ...media, // Copy all original properties
    name: media.customTitle ? media.customTitle : media.name, // Prioritize customTitle if it exists, otherwise use original name
    custom_title: media.customTitle // Explicitly add custom_title property
  };

  // Now, use mediaForDisplay when calling createSeriesGrid and setting dataset.media

  if (media.id && loadedMediaIds.has(media.id)) {
    // Use the modified media object for rendering
    const posterFragment = window.createSeriesGrid(imageBaseUrl, [mediaForDisplay], media.languages);
    const posterElement = posterFragment.firstChild;
    if (posterElement) {
    ...
    placeholder.className = "poster-placeholder aspect-[2/3] bg-gray-800 rounded-lg animate-pulse border border-gray-700 relative";
    
    // Use the modified media object for the placeholder dataset
    placeholder.dataset.media = JSON.stringify(mediaForDisplay);
