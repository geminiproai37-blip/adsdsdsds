...
async function createFavoriteCard(item) {
  // Find the content definition from the database in index.html
  const contentDefinition = window.contentDefinitionsFromDb.find(def => def.id == item.id && def.tipo === item.type);

  // If there's no definition, or no valid customUrl, do not create a card.
  if (!contentDefinition || !contentDefinition.customUrl || contentDefinition.customUrl === '#') {
    console.log(`Favorite item ${item.id} (${item.type}) skipped: No valid URL found in database.`);
    return null;
  }

  const finalUrl = contentDefinition.customUrl; // This is the required URL from the database
  const mediaType = item.type === 'movie' ? 'movie' : 'tv';
  const tmdbUrl = `https://api.themoviedb.org/3/${mediaType}/${item.id}?api_key=${tmdbApiKey}&language=es-ES`;

  let posterPath = '';
  let title = item.title || item.name || 'Título Desconocido';
  let rating = 0;
  let isAdultContent = item.isAdult || false;
  ...
  const imageUrl = posterPath ?
    `${window.imageBaseUrl}${posterPath}` :
  ...
  div.innerHTML = `
    <a href="${finalUrl}" class="block" onclick="event.target.closest('.delete-favorite-btn') ? event.preventDefault() : true">
      ...
      ${ratingBadge}
      ${isAdultContent ? `<div class="adult-badge">+18</div>` : ''}
