export function setupAboutAndDMCAModals() {
  ...
  const aboutModalTemplate = `
    <div id="about-modal" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center p-4 z-50 hidden">
      <div class="bg-slate-800 rounded-xl p-6 w-full max-w-md shadow-lg relative max-h-[90vh] overflow-y-auto my-auto">
        <button id="close-about-modal-button" class="absolute top-3 right-3 text-slate-400 hover:text-white transition-colors duration-200 z-10">
        ...
        <div class="text-slate-300 space-y-4 text-center">
          <p class="text-lg"><strong>Versión:</strong> <span class="text-orange-400">${window.globalAppVersion}</span></p>
          <p class="text-lg"><strong>Desarrollado por:</strong> <span class="text-orange-400">${window.globalDeveloperName}</span></p>
  ...
  if (aboutButton) {
    aboutButton.addEventListener("click", () => {
      ...
      if (aboutModal) {
        ...
        document.body.style.overflow = 'hidden'; // Previene el desplazamiento del fondo
